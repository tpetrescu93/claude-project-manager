...
2. **Reset to the up-to-date upstream default branch — skipping the fetch when the source is already current.** Repoint `origin` to the real upstream URL *first*. Before spawning, the extension resolves the default branch + upstream tip OID with one batched GraphQL call and compares it to the source's local default ref; **on a match** (the common case — the sync job keeps canonical repos current every minute) it passes `skipFetch`, and `clone.sh` does `git checkout -B <default> origin/<default>` straight from the **already-hardlinked local ref — no network fetch** (the local clone in step 1 mirrored the source's tip, which GraphQL just confirmed *is* upstream). **Otherwise** (source stale, non-GitHub, or GraphQL unavailable) it `git fetch origin <default>` as before. Either way it then `git remote set-head origin <default>` so the new clone's own `origin/HEAD` is correct for a future clone *of it*. Empty/unfetchable fallback: pick `main`/`master`/`develop` by local existence (never the source's checked-out branch).
3. `git checkout -b <branch>`.
4. **Replicate the source's untracked symlinks** at the same relative path — the dev-convenience links a fresh `git clone` doesn't carry: `.venv` → shared venv (root + per-subproject), `CLAUDE.local.md` → `~/.claude/<repo>.md` (per-repo Claude instructions), and anything similar. A symlink is copied verbatim (`cp -P`); a real `.venv` dir is symlinked back (`ln -s`). `find` prunes `.git`/`node_modules`, treats `.venv` as a unit (never descends into a real one to copy its internal `bin/python*`), and skips links the clone already reproduced (tracked ones). Instant — no `uv sync` for same-deps branches, and the clone keeps its per-repo Claude guidance.
5. **Eagerly start a primed Claude tmux session** (`claude --dangerously-skip-permissions; exec bash -l`) — same as fork, but a *fresh* Claude rather than a resumed one. So opening the cloned project later just *attaches* to an already-booted Claude instead of cold-starting it. No race with first-open auto-open: the pending file (step 6, which makes the project appear in the sidebar) is written *after* the session is created, so the session is always primed by the time the project is clickable; and `openTmuxSession`'s `attach || new` just attaches.
...
**JS deps are intentionally not installed on clone.** Clone time stays uniform/fast, most clones never run the JS app, and when deps *are* needed the project's own package manager (`pnpm`/`yarn`/`npm`) installs them then — faithful to its lockfile, rather than `bun install` doing a non-faithful fresh resolve of a pnpm/yarn project. (Python is the exception: step 4's `.venv` symlink makes its deps free; JS `node_modules` can't be shared that way, so it's left on-demand. Earlier this ran `bun install` on any lockfile/`package.json`, which did a cold-cache 89s resolve on a pnpm repo — dropped in favour of on-demand.)

**Why step 2 resolves the default from the *source's* cached `origin/HEAD`, not the new clone's, and not a network lookup.** A local `git clone <folder>` sets the **new** repo's `origin/HEAD` to mirror whatever branch the **source had checked out** — so reading the *clone's* `git symbolic-ref refs/remotes/origin/HEAD` (an early approach) resolved to the source's *feature* branch, and the clone silently inherited that branch (its feature commits, or a behind-by-N default) instead of real master. The intermediate fix asked the **upstream** directly via `git ls-remote --symref <upstream> HEAD` — correct, but ~1.8s per clone on a repo with many refs (paydays-api advertises ~52k), since the cost is server-side upload-pack setup that protocol v2 / ref-prefix filtering / fetch-by-SHA don't escape. The current approach reads the **source's** `origin/HEAD` — the source repo's tracking of *its* upstream's default, which is the true default and is kept correct by the repo-sync job (canonical repos), by `git clone` (freshly added repos), and by the `set-head` stamp each clone now applies to itself. Instant, no per-clone network lookup; the `ls-remote` is gone, with only the empty/unfetchable-cache fallback (local `main`/`master`/`develop` picker) left for the offline case. Verified end-to-end: source with its default cached correctly → clone lands on the real default, a source feature-branch commit dropped.

**Why the clone can skip the fetch entirely.** Step 1's local `git clone <folder>` already hardlinks the source's objects, so the new clone's `origin/<default>` *is* the source's default tip — the only reason to fetch is in case the source is behind upstream. So before spawning, the extension does one batched GraphQL call (`defaultBranchRef { name target { oid } }`, the same `bulkFetchDefaultBranches` the repo-sync job uses) and compares the upstream tip OID to the source's local `refs/heads/<default>`. If they match, the clone already has the upstream tip and the ~1.7s `git fetch` is pure waste — so it's skipped and the default is checked out from the local ref. This is the *verified* form of "trust the source is fresh": it only skips when GraphQL confirms it, falling back to a real fetch otherwise — so a stale source still self-corrects. Detection is ~0.5s (and GitHub-only; non-GitHub sources always fetch). Benchmarked: skip path ~2.96s with zero network for the branch (no `FETCH_HEAD`), vs ~3.7s+ fetching — and the skip path is network-immune, so the gap widens when GitHub is slow. Nearly always hit when cloning a canonical Git-view repo, since the sync job keeps those at the upstream tip.
...
- **Auto-named, one click.** The default fork doesn't prompt — it derives the name from the source via a `-ptN` convention (`suggestForkName`): bump a trailing `-ptN` (`foo-pt2` → `foo-pt3`), or append `-pt2` when there's none (the unsuffixed original is implicitly pt1, so `foo` → `foo-pt2`). `nextFreeForkName` then keeps bumping while a folder or `projects.json` entry with that name already exists, so repeated forks never collide (`foo-pt2` → `foo-pt3`, → `foo-pt4`, …). The chosen name becomes both the folder (`<repo>-<name>`) and the git branch.
- **"Fork Project As…"** is a separate right-click entry (Favorites + Git) that prompts for a custom name, prefilled with that same next-free suggestion and validated (`validateBranchName`, must differ from source). Use it when the `-ptN` sequence name isn't what you want. Archived fork is always one-click auto.
...
- **Resume**: the detached bash script starts `claude --resume <session-id>` in a detached tmux session after the clone completes. Switching to the fork and "Open Tmux Session" attaches to the already-running resumed session (first-open auto-open finds the existing session and attaches rather than spawning a fresh Claude).
- **Orienting prompt on resume.** The resumed conversation happened in the *source* folder with its own branch and (often) uncommitted work, but the fork is a clean checkout on a fresh branch off latest default — so Claude is passed a one-time park message as the positional `[prompt]` to `claude --resume` (interactive resume submits it as the first turn, no `send-keys` timing): *"Heads up: this is a forked session… the working directory is a fresh clone on a new branch… any uncommitted changes or commits from earlier are NOT present here… Do not try to recreate that prior state. Await my next instruction."* It stops Claude from acting on stale file/git assumptions and parks it for your next instruction. Lives in `clone.sh`'s session block, so it applies to **fork and promote** (both resume into a fresh git checkout), never to plain clone (no session) or investigation→investigation fork (empty scratch dir, no git reset). Single-quoted in the `-c` string, so the message contains no apostrophes.
...
- **Eager Claude start**: creating an investigation also spawns a detached tmux session running `claude --dangerously-skip-permissions; exec bash -l`, so opening it attaches to an already-warmed Claude rather than a cold bash prompt. (If tmux is unavailable the spawn is a no-op and "Open Tmux Session" starts it on demand.)
...
- **Promote** (`$(rocket)`): turns a scratch investigation into real work. Picks a **repo from the curated Git list**, runs the **clone flow** (same detached bash script as Clone/Fork) into a fresh `~/projects/<name>` folder, **carries the Claude session across** (transcript copied + `cwd`-rewritten), resumes it in a tmux session, then **tears down the scratch investigation** (kill tmux, `rm -rf` folder, pop the `projects.json` entry) — the investigation row is gone and replaced by the new full project entry, which gets PR/CI status polling like any other project. Runs in the background — safe to switch workspaces while it runs.
...
**Delete Project (Permanently).** A right-click command on Favorites rows (`_projectManager.deleteProjectFromDisk`) deletes a live project outright — modal confirm ("…remove its folder from disk? This cannot be undone."), then `projectStorage.pop` + `forgetTmuxAutoOpened` + `rm -rf` + kill tmux (shared `removeProjectFromDisk`). This replaces upstream's dormant `_projectManager.deleteProject` (a soft "remove from list only", hidden behind `when:false` and never exposed), which was removed entirely to avoid two delete commands with different semantics.

**Deleting the *currently-open* project** can't `rm -rf` its own workspace folder, and switching away reloads the extension host — so the removal is **deferred across the reload**. On confirm: record `pendingProjectDelete = {name, rootPath}` in `globalState`, then `workbench.action.closeFolder` (lands in an empty window; the extension stays activated via `onStartupFinished`). On the next activation, `completePendingProjectDelete` (called from `registerArchiveCommands`) checks the flag, confirms we're no longer in that folder, **clears the flag first** (so a mid-delete crash can't loop-retry), runs `removeProjectFromDisk`, and reveals the Project Manager sidebar (`workbench.view.extension.project-manager`) so you land back on the Projects list. Crash-safe: the flag persists on disk, so an interrupted delete completes on the next launch. (Earlier this was a hard block — "can't delete, it's open"; the deferred approach replaces the modal refusal, since deleting the open folder inline leaves VS Code in a broken state — empty Explorer, stale editors, dead terminal cwd.)
...
### Git view = a curated list of canonical repos
The Git section shows an **explicit, extension-managed list** of canonical repos — not whatever git repos happen to be on disk. (This replaced the old auto-detect-everything-in-`~/projects` + pin/filter approach, which existed only to hide the dozens of `<repo>-<branch>` working copies from the noise.)

- **Canonical repos live in `~/projects/.repos`** (`REPOS_BASE = path.join(PROJECTS_BASE, ".repos")`); working copies (clones/forks/investigations) live in `~/projects`. The two are cleanly separated, so the Git view never shows working-copy clutter.
- **The list is stored in `globalState` (`gitRepoList`)**, an ordered array of rootPaths. `CuratedGitLocator` (a `CustomProjectLocator` subclass) feeds the existing `AutodetectProvider` from this list instead of scanning `baseFolders` — so drag-reorder, the title count, and all per-row actions keep working, but the data source is the list, not disk. `baseFolders`/`maxDepthRecursion` are now inert for Git.
- **Seeded once** from the repos already in `~/projects/.repos` (marker-guarded so an intentionally-emptied list isn't re-seeded).
- **Add Git Repo** (`$(add)` on the Git view title): prompts for a git URL, `git clone`s it into `~/projects/.repos`, appends it to the list, refreshes. (If the target folder already exists it warns and aborts — there's no "register an existing on-disk repo" path yet.)
- **Remove from List** (`$(close)`, inline + right-click): **non-destructive** — drops the repo from the list so it leaves the Git view; the folder on disk is untouched. (No `rm -rf`.)
- **Inline buttons** per Git row: `$(repo-forked)` Clone → `$(terminal)` Open Tmux → `$(close)` Remove. Pinning (`gitPinnedRepos`, "show pinned only") was removed entirely.
- **Promote** picks from this list (was the pinned set).

### Keep canonical repos fresh — background sync
A `setInterval` job (`syncGitRepos.ts`) keeps every repo in the curated Git list (`~/projects/.repos/*`) fast-forwarded to its upstream default branch, so the local object store stays current and per-clone/fork fetches transfer little or nothing.

- **Cadence:** every `projectManager.git.autoSync.intervalMinutes` (default **1 min** — affordable because detection is one ~0.5s GraphQL call that does no git network when nothing changed) while a VS Code window is open, plus a one-shot kickoff on activation **gated by a `gitRepoLastSync` globalState throttle**. The ext host restarts on every project switch (`openFolder` reloads the window), so without the throttle each switch would trigger a full sweep; the throttle caps it at one sweep per interval regardless of restarts. In-extension timer only — not an OS daemon, so syncing stops when all windows close. `projectManager.git.autoSync.enabled` (default true) is the off switch; an in-progress guard prevents overlapping sweeps.
- **Detection is one batched GraphQL call**, not a per-repo `git ls-remote`. Each repo's GitHub owner/name (parsed from its origin URL) goes into a single aliased `repository(...) { defaultBranchRef { name target { oid } } }` query (reusing the PR-poller's token + fetch plumbing in `githubBulkFetch.ts`) — **~0.5s for all repos at once**, versus ~1.8s of SSH upload-pack *per* repo (the server's 50k-ref advertisement on a repo like paydays-api dominates, and protocol v2 / ref-prefix / fetch-by-SHA don't escape it — benchmarked). Each returned tip OID is compared to the local default ref (`git rev-parse refs/heads/<default>`, instant): **equal → skip, zero git network**; only a mismatch pays a fetch. Since default branches rarely move, the common sweep does no git network at all. Non-GitHub remotes — or a GraphQL/auth failure — fall back to the git path (`git remote set-head origin -a` → `refs/remotes/origin/HEAD`).
- **Fast-forward is non-destructive,** and only runs when a repo's tip actually moved: `fetch origin <default>`, then — *on the default branch + clean:* `merge --ff-only`; *on a feature branch:* fast-forward the **local default ref** via `fetch origin <def>:<def>` (no working-tree or branch change); *dirty (tracked changes) or diverged:* fetch only, skip the ff. **Never** switches branches, resets, or touches submodules. Untracked files don't block the ff (`status --porcelain --untracked-files=no`) — a fast-forward can't clobber them.
- **Self-seeding:** calls `seedGitRepoListOnce()` before reading the list, so it runs even if the Git view was never opened this session (otherwise the list stays empty until the view's first refresh and the job silently does nothing).
- **Quiet:** deliberately does **not** call `refreshGitProjects` — that pops a progress notification + "refreshed!" toast on every call, intolerable once a minute, and the curated-list membership doesn't change when branches move. Sweeps log to a `Project Manager: Repo Sync` output channel; a hidden `_projectManager.syncGitRepos` command runs a manual sweep with a summary toast.
...
```
tmux attach -t "=<session>" 2>/dev/null || tmux new -s "<session>" bash -lic 'claude --dangerously-skip-permissions; exec bash -l'
```

Session name = `path.basename(rootPath)` with dots → dashes (same string the thinking-detection code uses for `capture-pane`, so the two stay in sync). Works on any system with bash + tmux.

**Auto-starts Claude on a fresh session.** Attaching to an existing session is left untouched (`-t "=<session>"`, exact-match), but a *newly created* session launches `claude --dangerously-skip-permissions` immediately, then `exec bash -l` so quitting Claude drops into a **login** shell — `~/.bash_profile` is sourced, so you keep your normal prompt/profile — rather than killing the session. (`exec bash` without `-l` was a non-login shell and lost the profile; that was the bug.) The `; exec bash -l` tail is applied consistently everywhere Claude is auto-launched: this command, investigation creation/fork, and the `clone.sh` fork/promote resume.
...
On *subsequent* opens of the same project, the command attaches to the persistent tmux session that survives VS Code restarts, workspace switches, and even VS Code crashes (it's its own OS process) — so Claude is only auto-launched when the session is genuinely new.

**Auto-open the tab on first open (once per project).** The tmux tab is auto-opened the **first time a project is ever the active workspace** — by any path: a sidebar switch, `File > Open Folder`, or VS Code cold-launching into it. This is decided entirely on the new host's activation (no flag is carried across the `openFolder` reload): it reads `workspaceFolders[0]` and a persistent `globalState["tmuxAutoOpened"]` set of rootPaths. If the path isn't in the set, it adds it and fires `openTmuxSession` (immediately if `window.state.focused`, else deferred via `window.onDidChangeWindowState` so the workbench is fully rendered first); if it's already there, nothing is forced — so closing the tab and switching back won't reopen it. The set is pruned whenever a project/investigation is deleted (`forgetTmuxAutoOpened` in `utils/tmuxAutoOpen.ts`), preventing leaks and letting a project recreated at the same path count as new.

Earlier this was gated on `tmux has-session` via a transient `autoStartTmux` flag, on the assumption that an existing session meant the tab would be restored. That was wrong — VS Code *revives* (relaunches) rather than *reconnects* terminals on a folder switch, so a pre-existing session left you with no tab. Reading the persistent set on activation is both simpler (one flag, no cross-reload handoff) and correct.
