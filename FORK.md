...
**Delete Project (Permanently).** A right-click command on Favorites rows (`_projectManager.deleteProjectFromDisk`) deletes a live project outright — modal confirm ("…remove its folder from disk? This cannot be undone."), then `projectStorage.pop` + `forgetTmuxAutoOpened` + `rm -rf` + kill tmux. Blocked if the project is open in the current window. This replaces upstream's dormant `_projectManager.deleteProject` (a soft "remove from list only", hidden behind `when:false` and never exposed), which was removed entirely to avoid two delete commands with different semantics.
