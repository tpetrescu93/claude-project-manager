...
4. **Replicate every `.venv` the source has** at the same relative path — the root `.venv` plus any per-subproject one (a monorepo may keep several). A symlink is copied verbatim (`cp -P`), a real dir is symlinked back (`ln -s`). `find -maxdepth 2 -name .venv -prune` so it never descends into a real venv's thousands of files. Instant — no `uv sync` needed for same-deps branches.
5. If `yarn.lock` or `package-lock.json` exists → `bun install && rm -f bun.lock bun.lockb`.
6. **Eagerly start a primed Claude tmux session** (`claude --dangerously-skip-permissions; exec bash -l`) — same as fork, but a *fresh* Claude rather than a resumed one. So opening the cloned project later just *attaches* to an already-booted Claude instead of cold-starting it. No race with first-open auto-open: the pending file (step 7, which makes the project appear in the sidebar) is written *after* the session is created, so the session is always primed by the time the project is clickable; and `openTmuxSession`'s `attach || new` just attaches.
7. Detect `repoName` from the (already-repointed) upstream URL and store just the branch in `projects.json` (strip `<repo>-` prefix if present).
